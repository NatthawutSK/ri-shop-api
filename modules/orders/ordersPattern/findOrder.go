...
func (b *findOrderBuilder) buildWhereDate() {
	if b.req.StartDate != "" && b.req.EndDate != "" {
		...
		query := fmt.Sprintf(`
		AND "o"."created_at" BETWEEN DATE($%d) AND ($%d)::DATE + 1`, // +1 คือเพื่อให้เอาวันที่สุดท้ายด้วย
			b.lastIndex+1,
			b.lastIndex+2,
		)
		temp := b.getQuery()
...
func (b *findOrderBuilder) setLastIndex(index int) {
	b.lastIndex = index
}
...
func (b *findOrderBuilder) resetQuery() {
	b.query = ""
	b.values = make([]any, 0)
	b.lastIndex = 0
}
...
func (en *findOrderEngineer) FindOrder() []*orders.Order {
	...
	en.builder.buildSort()// sort ต้องอยู่ก่อน paginate เสมอ
	...
	if err := en.builder.getDb().Get(&bytes, en.builder.getQuery(), en.builder.getValues()...); err != nil {
		log.Printf("find orders failed: %v\n", err)
		return make([]*orders.Order, 0)
	}

	if err := json.Unmarshal(bytes, &ordersData); err != nil {
		log.Printf("unmarshal orders failed: %v\n", err)
		return make([]*orders.Order, 0)
	}


	en.builder.resetQuery()
	return ordersData
}
